...
- (NSString *)picturesDirectory
{
	...
	NSString *result =
    [self
     findOrCreateDirectory:NSPicturesDirectory
     inDomain:NSUserDomainMask
     appendPathComponent:executableName
     error:&error];
	if (!result)
	{
		NSLog(@"Unable to find or create pictures directory:\n%@", error);
	}
	return result;
}
...
- (NSString *)musicDirectory
{
	...
	NSString *result =
    [self
     findOrCreateDirectory:NSMusicDirectory
     inDomain:NSUserDomainMask
     appendPathComponent:executableName
     error:&error];
	if (!result)
	{
		NSLog(@"Unable to find or create music directory:\n%@", error);
	}
	return result;
}
...
- (NSString *)moviesDirectory
{
	...
	NSString *result =
    [self
     findOrCreateDirectory:NSMoviesDirectory
     inDomain:NSUserDomainMask
     appendPathComponent:executableName
     error:&error];
	if (!result)
	{
		NSLog(@"Unable to find or create movies directory:\n%@", error);
	}
	return result;
}
